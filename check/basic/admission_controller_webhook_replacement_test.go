...
func TestWebhookCheckMetaReplacement(t *testing.T) {
...
func TestWebhookCheckRegistrationReplacement(t *testing.T) {
	webhookCheck := &webhookReplaementCheck{}
	check, err := check.Get("admission-controller-webhook-replacement")
...
func TestWebhookError(t *testing.T) {
	tests := []struct {
		...
		{
			name: "namespace label selector does not match check-system",
			objs: webhookTestObjects(
				ar.Fail,
				&metav1.LabelSelector{
		...
		{
			name: "namespace OpExists expression selector does not match check-system",
			objs: webhookTestObjects(
				ar.Fail,
				&metav1.LabelSelector{
		...
		{
			name: "namespace OpDoesNotExist expression selector does not match check-system",
			objs: webhookTestObjects(
				ar.Fail,
				&metav1.LabelSelector{
		...
		{
			name: "namespace OpIn expression selector does not match check-system",
			objs: webhookTestObjects(
				ar.Fail,
				&metav1.LabelSelector{
		...
		{
			name: "namespace OpNotIn expression selector does not match check-system",
			objs: webhookTestObjects(
				ar.Fail,
				&metav1.LabelSelector{
		...
		{
			name: "webhook applies to its own namespace and check-system",
			objs: webhookTestObjects(
				ar.Fail,
				&metav1.LabelSelector{},
				ar.WebhookClientConfig{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			d, _, err := webhookCheck.Run(test.objs)
			assert.NoError(t, err)
...
func webhookErrors() []check.Diagnostic {
